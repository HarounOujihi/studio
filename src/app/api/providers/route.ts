...
// GET - List providers with pagination and search
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate unique reference (PRV00001 format)
    const lastProvider = await prisma.provider.findFirst({
      where: { idOrg, idEtb },
      orderBy: { reference: "desc" },
    });

    const lastRef = lastProvider?.reference || "PRV00000";
    const refNum = (parseInt(lastRef.replace("PRV", "")) || 0) + 1;
    const reference = `PRV${String(refNum).padStart(5, "0")}`;
    ...
    // Create provider
    const providerId = `provider-${nanoid()}`;
