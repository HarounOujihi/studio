...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      taxes: taxes.map((t) => ({ ...t, createdAt: t.createdAt?.toISOString() || null })),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { idOrg, idEtb, designation, value, taxType } = body;
    ...
    const lastTax = await prisma.tax.findFirst({ where: { idOrg, idEtb }, orderBy: { reference: "desc" } });
    const lastRef = lastTax?.reference || "TAX00000";
    const refNum = (parseInt(lastRef.replace("TAX", "")) || 0) + 1;
    const reference = `TAX${String(refNum).padStart(5, "0")}`;

    const taxId = `tax-${nanoid()}`;
    const tax = await prisma.tax.create({
      data: { id: taxId, idOrg, idEtb, reference, designation: designation || null, value: value || null, taxType: taxType || null },
    });

    return NextResponse.json({ tax, success: true });
  } catch (error) {
    console.error("Error creating tax:", error);
    return NextResponse.json({ error: "Erreur lors de la création de la taxe" }, { status: 500 });
  }
}
